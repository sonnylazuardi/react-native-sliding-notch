...
class SlidingDownPanel extends React.Component {
  static propTypes = {
  ...
  componentWillMount() {
    if (__DEV__) {
      deprecated(
        ...
        "SlidingDownPanel#contentStyle is deprecated. " +
  ...
  transitionTo(value, onAnimationEnd = () => {}) {
    ...
    Animated.timing(this._translateYAnimation, animationConfig).start(
      onAnimationEnd
    );
  }
  ...
  render() {
    ...
    console.log("VH", visibleHeight);

    const animatedContainerStyles = [
      styles.animatedContainer,
      this.props.contentStyle,
      transform,
      { height, top: 100, bottom: 0 }
    ];
...
export default SlidingDownPanel;
